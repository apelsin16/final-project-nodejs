...
export const createUser = async ({ name, email, password }) => {
    ...
    if (existingUser) {
        throw HttpError(409, 'Email already exists. Please use a different email address.');
    }
...
export const modifyUserAvatar = async (id, avatarURL) => {
    ...
    if (!user) throw HttpError(401, 'Authentication required. Please log in and try again.');
...
export const getCurrentUserDetailed = async user => {
    ...
    const [ownRecipesCount, favoriteRecipesCount, followersCount, followingCount] = await Promise.all([
...
export const followUser = async (followerId, followingId) => {
    if (followerId === followingId) {
        throw HttpError(400, 'Cannot follow yourself. Please select a different user to follow.');
    }
    ...
    try {
        ...
        throw HttpError(404, 'User not found. Please check the user ID and try again.');
    }
    ...
    if (existingFollow) {
        throw HttpError(409, 'Already following this user. You are already subscribed to this profile.');
    }

    await Follow.create({ followerId, followingId });

    return {
        message: 'Successfully followed user. You are now subscribed to their updates.',
        following: {
            id: userToFollow.id,
...
export const unfollowUser = async (followerId, followingId) => {
    ...
    try {
        ...
        throw HttpError(404, 'Not following this user. You are not subscribed to this profile.');
    }
    ...
    return {
        message: 'Successfully unfollowed user. You are no longer subscribed to their updates.',
    };
};
