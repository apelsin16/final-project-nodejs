...
const getFollowingController = async (req, res) => {
  ...
  const followingData = await getFollowingByUserId(
    userId,
    parseInt(page),
    parseInt(limit)
  );
...
const getFollowersController = async (req, res) => {
  ...
  const followersData = await getFollowersByUserId(
    userId,
    parseInt(page),
    parseInt(limit)
  );
...
const updateUserAvatarController = async (req, res) => {
  if (!req.file) {
    throw HttpError(
      400,
      'No file provided. Please upload an image file and try again.'
    );
  }
